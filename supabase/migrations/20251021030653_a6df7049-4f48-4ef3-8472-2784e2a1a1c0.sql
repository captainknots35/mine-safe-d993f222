-- Update Lesson 1 with embedded videos at strategic points
UPDATE lessons 
SET content_data = jsonb_build_object(
  'sections', jsonb_build_array(
    jsonb_build_object(
      'type', 'content',
      'content', '## The Foundation of Mine Safety: The Mine Act and MSHA''s Mission
...
|-----------|-------------|
| Right to Training | Receive all required safety and health training during work hours, paid at regular rate |
| Right to a Safe Workplace | Work environment free from recognized hazards |
| Right to Report Hazards | Report conditions without fear of retaliation (Section 105(c) protection) |
| Right to Refuse Unsafe Work | Refuse work you believe is unsafe after notifying supervisor |
| Right to Participate | Walk-around rights with inspectors, speak confidentially with MSHA |
| Right to Compensation | Payment if idled due to MSHA withdrawal orders |
...
These three roles—the miner, the supervisor, and the miners'' representative—form an interdependent safety system. Your legally protected right to report a hazard directly triggers your supervisor''s legal responsibility to investigate and correct it. The miners'' representative serves as a crucial advocate and an additional channel for communication if that initial process fails. The supervisor''s personal liability provides a powerful motivation to take your concerns seriously. This is not simply a list of job descriptions; it is a dynamic system of checks and balances designed to ensure that safety issues are identified, communicated, and resolved before an accident can occur. Your voice is a legally protected and essential component of this structure.'
    ...
    jsonb_build_object(
      'type', 'video',
      'title', 'MSHA Part 46 - Line of Authority',
      'description', 'These three roles—miner, supervisor, and miners'' representative—work together. This video shows how that communication is supposed to flow to resolve a safety issue.',
      'videoUrl', 'https://www.youtube.com/watch?v=8Ur9PPEFMhU'
    )
  )
)
WHERE id = '68f1c721-2cee-42e4-8c85-62d4af9ffb43';

-- Update Lesson 2 with embedded video
UPDATE lessons 
SET content_data = jsonb_build_object(
  'sections', jsonb_build_array(
    jsonb_build_object(
      'type', 'content',
      'content', '## Anatomy of a Surface Mine: Learning the Language of the Land
...
To work safely, you must be able to understand instructions and communicate accurately with your coworkers. This requires learning the specific vocabulary used to describe the mine''s physical environment. Let''s build a mental map of a typical surface mine.'
    ...
    jsonb_build_object(
      'type', 'video',
      'title', 'Anatomy of a Quarry',
      'description', 'To understand the language of the land, it helps to see it from above. This video will show you what highwalls, benches, berms, and haul roads look like in a real operation.',
      'videoUrl', 'https://www.youtube.com/watch?v=LD-vNX6_QdE'
    ),
    jsonb_build_object(
      'type', 'content',
      'content', '**The Pit/Quarry:** This is the heart of the operation—the large, open excavation where the mineral is extracted. These can be vast areas, sometimes hundreds of feet deep.

**Highwall:** This is the unexcavated, cliff-like face of the pit or quarry. Highwalls can be unstable and are a significant source of ground-fall hazards.
...
**Berm:** This is a raised pile or mound of material, typically built along the outer edge of a haul road or at a dumping location. Its purpose is to act as a safety barrier to help prevent vehicles from going over the edge.

**Haul Road/Ramp:** These are the primary arteries of the mine, connecting the pit to the processing plant. They are carefully constructed and maintained to handle the weight of massive haul trucks and are often wide enough for two-way traffic.

**Overburden:** This is the layer of topsoil, clay, and non-valuable rock that sits on top of the mineral deposit. It must be stripped away and stockpiled before mining can begin.

**Stockpiles:** These are large, cone-shaped piles of either raw material from the pit or finished, processed product waiting to be shipped. The natural slope of these piles is called the "angle of repose," and they can be hazardous to work around.

**Processing Plant (Mill/Preparation Plant):** This is the industrial facility where the raw material is taken to be crushed, screened (sized), washed, and sorted into final, saleable products.

**Water Impoundments/Settling Ponds:** These are engineered dams and ponds used to store water for processing or to contain fine waste materials, known as slurry or tailings. They present unique hazards and are regulated by MSHA.

### Table 2: Key Mining Terminology and Definitions

| Term | Definition |
|------|------------|
| Pit/Quarry | Large open excavation where minerals are extracted |
| Highwall | Unexcavated cliff-like face; potential ground-fall hazard |
| Bench | Stepped terraces cut into highwall for safe equipment operation |
| Berm | Safety barrier mound along road edges |
| Haul Road | Primary roads for heavy equipment traffic |
| Overburden | Non-valuable material above mineral deposit |
| Stockpile | Cone-shaped piles of material |
| Angle of Repose | Natural slope angle of stockpiled material |
| Processing Plant | Facility for crushing, screening, and sorting material |
| Sump | Low point where water collects |'
    )
  )
)
WHERE id = 'a1b2c3d4-e5f6-7890-abcd-001122334455';

-- Update Lesson 3 with embedded videos for virtual tour
UPDATE lessons 
SET content_data = jsonb_build_object(
  'sections', jsonb_build_array(
    jsonb_build_object(
      'type', 'content',
      'content', '## An Immersive Journey Through Mining Operations (30 CFR § 46.5(b)(1))
...
Regardless of how it is extracted, the material''s journey continues to the processing plant. The haul truck dumps its load into a hopper, which feeds a series of conveyor belts. You can hear the loud, vibrating noise of the screen decks, which act like giant sieves to separate the material by size. Larger rocks are sent to crushers to be broken down, while the sand and gravel are washed to remove clay and silt before being sorted into various stockpiles of finished product. Because these operations often involve large open pits and water-filled dredges, the primary hazards are related to mobile equipment traffic, the stability of water impoundments, and the mechanical dangers of the processing plant.'
    ...
    jsonb_build_object(
      'type', 'video',
      'title', 'Super Giant Sand Plant: Extracting Sand From Rivers and Quarries',
      'description', 'Now let''s see a large-scale sand and gravel operation in action, from dredging in the river to processing at the plant.',
      'videoUrl', 'https://www.youtube.com/watch?v=TzcKBwS58Kg'
    ),
    jsonb_build_object(
      'type', 'content',
      'content', '## Virtual Stop 2: The Surface Stone (Limestone) Quarry
...
Unlike sand and gravel, this hard rock cannot simply be dug out. The extraction process here is a cycle of drilling and blasting. A large drill rig moves onto a bench and drills a precise pattern of deep holes into the rock. These holes are then carefully loaded with explosives. After the area is cleared and warning signals are sounded, the blast occurs, fracturing thousands of tons of rock. This fragmented rock is then scooped up by a massive hydraulic shovel or front-end loader and placed into haul trucks for transport out of the pit. The dominant hazards in this environment are directly related to this process: the stability of the highwalls (ground control), the dangers of handling and using explosives (flyrock), and the operation of heavy equipment on steep haul roads.'
    ...
    jsonb_build_object(
      'type', 'video',
      'title', 'Exploring Surface Stone Quarry - Heavy Equipment Working',
      'description', 'Hard rock mining is a cycle of drilling, blasting, and hauling. This video takes us through that entire process step-by-step.',
      'videoUrl', 'https://www.youtube.com/watch?v=NGw2VhXzQGo'
    ),
    jsonb_build_object(
      'type', 'content',
      'content', '## Virtual Stop 3: The Surface Clay Mine
...
At the processing plant, the raw clay is first sent through crushers and mills to grind it into a fine powder. It is then often mixed with water and washed to remove impurities like sand. A critical step is drying; the wet clay slurry is fed into large, rotating dryers to remove moisture. Finally, it is screened or classified by particle size to meet the specifications for its end use, such as in ceramics or construction materials. The fine, powdery nature of this material means that a primary hazard is respiratory; the creation and control of airborne dust is a major safety and health concern.'
    ...
    jsonb_build_object(
      'type', 'content',
      'content', '## Virtual Stop 4: The Shell Dredging Operation
...
We are now on a floating vessel, a hydraulic dredge. The entire mining operation takes place on the water. You can feel the constant hum of the engines and pumps. A long frame called a "ladder" is lowered from the dredge to the seabed. At the end of the ladder is a rotating cutterhead and a suction intake. The cutterhead breaks up and agitates the shells and sediment on the bottom, mixing them with water to create a "slurry." This slurry is then drawn into the intake by a powerful pump located on the dredge. From the dredge, the slurry is pumped through a large, flexible pipeline, often floated on pontoons, which carries the material all the way to a processing facility on the shore. This environment is fundamentally different from a land-based mine. The most significant and immediate hazards are water-based: the risk of drowning, slips and falls on wet decks, the stability of the vessel itself, and exposure to rapidly changing weather conditions.'
    ...
    jsonb_build_object(
      'type', 'video',
      'title', 'Trailing Suction Hopper Dredgers - How Does It Work?',
      'description', 'This video provides a clear visual demonstration of how a dredging operation extracts material from the water.',
      'videoUrl', 'https://www.youtube.com/watch?v=2F8rS7t6hAw'
    ),
    jsonb_build_object(
      'type', 'content',
      'content', '## Virtual Stop 5: The Colloidal Phosphate Site
...
Because the material is soft and already at the surface, the extraction method is straightforward. Loaders and trucks are used to mine the clay-like material and move it to a stockpile, much like a surface clay mine. Processing is also minimal compared to other commodities. It typically involves drying and grinding the material before it is packaged and sold as a slow-release organic fertilizer. Similar to clay mining, the fine and dusty nature of the material makes respiratory protection a key health and safety consideration.'
    ...
    jsonb_build_object(
      'type', 'video',
      'title', 'How Phosphate Is Produced',
      'description', 'This video shows the phosphate mining and processing operations, including the massive equipment and material handling involved.',
      'videoUrl', 'https://www.youtube.com/watch?v=rQiHzUwJW_8'
    )
  )
)
